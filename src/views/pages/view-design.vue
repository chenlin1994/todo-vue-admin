<template>
  <div>
    <div class="list">
      <h2>任务列表</h2>
      <Button type="primary" icon="md-add" size="large" @click="addTask"
        >添加任务</Button
      >
    </div>
    ...
    <Table :loading="loading" border :columns="columns" :data="tableData">
      <template slot-scope="{ row, index }" slot="action">
        ...
        <Button
          ...
          v-if="row.status != 2"
          >{{
            row.status == 0 ? '完成' : row.status == 1 ? '待办' : null
          }}</Button
        >
        <Button
          type="error"
    ...
    >
      ...
      <div class="demo-drawer-footer">
        <Button
          ...
          >{{ textBtn }}</Button
        >
        <Button @click="handleReset('formValidate')" style="margin-right: 15px"
          >重置</Button
        >
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            const row = params.row
            const text =
              row.status == 0 ? '待办' : row.status == 1 ? '完成' : '删除'
            return h('div', text)
          },
          filters: [
            {
  ...
  methods: {
    ...
    toggleFav(row, index) {
      if (row.status == 2) {
        ...
        // is_major: 0:不重要 1:重要
        this.tableData[index].is_major =
          this.tableData[index].is_major === 0 ? 1 : 0
